  ...
  var decoder = {
    ...
    initAudioPlanar: function (channels, samplerate) {
      ...
      this.playAudioPlanar = function (data, len) {
        ...
        if (remain) {
          len = 1024 - remain;
          if (frameCount >= len) {
            outputArray[0] = Float32Array.of(
              ...buffer[0],
              ...origin[0].subarray(0, len)
            );
            if (channels == 2)
              outputArray[1] = Float32Array.of(
                ...buffer[1],
                ...origin[1].subarray(0, len)
              );
            postMessage(
              { cmd: "playAudio", buffer: outputArray },
    ...
    play: function (url) {
      ...
      if (url.indexOf("http") == 0 && url.indexOf(".flv") != -1) {
            ...
            var fetchNext = function () {
              reader
                ...
                .catch(function (e) {
                  ...
                  if (
                    e.toString().indexOf("The user aborted a request") === -1
                  ) {
        ...
        this.opt.debug &&
          postMessage({ cmd: "printErr", text: "目前只支持flv" });
        ...
        if (
          ...
          this.draw = function (compositionTime, y, u, v) {
            render(
              w,
              h,
              Module.HEAPU8.subarray(y, y + size),
              Module.HEAPU8.subarray(u, u + qsize),
              Module.HEAPU8.subarray(v, v + qsize)
            );
