...
class Jessibuca {
  constructor(options) {
    // 合并初始设置和自定义设置
    this._opt = { ...DEFAULT_OPTIONS, ...options };
    // 缓存容器dom => this.$container
    const containerEle = options.container;
    if (typeof containerEle === "string") {
      this.$container = document.querySelector(containerEle);
    } else {
   ...
   */
  log() {
    this._opt.debug && console.log(...arguments);
  }
}
